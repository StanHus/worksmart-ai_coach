...
# Using WorkSmart telemetry instead of pynput
PYNPUT_AVAILABLE = False  # Disabled - using WorkSmart data
...
class ClaudeClient:
    ...
    async def analyze_telemetry(self, telemetry_data: Dict, context: str = "") -> Optional[Dict]:
        ...
        logger.debug(f"🔍 Claude analyzing telemetry: {telemetry_data}")
                ...
                async with httpx.AsyncClient(timeout=30.0) as client:
                    ...
                    if response.status_code == 200:
                        ...
                        try:
                            ...
                            logger.debug(f"✅ Claude JSON parsed: {parsed_result}")
                            return parsed_result
                        except json.JSONDecodeError:
                            # Try removing markdown wrapper
                            if content.strip().startswith('```json') and content.strip().endswith('```'):
                                try:
                                    # Extract JSON from markdown code block
                                    json_content = content.strip()[7:-3].strip()  # Remove ```json and ```
                                    parsed_result = json.loads(json_content)
                                    logger.debug(f"✅ Claude JSON parsed (after unwrapping): {parsed_result}")
                                    return parsed_result
                                except json.JSONDecodeError as e:
                                    logger.warning(f"❌ Claude returned invalid JSON even after unwrapping: {e}")
                                    logger.warning(f"Raw content: {content}")
                                    return None
                            else:
                                logger.warning(f"❌ Claude returned invalid JSON: {content}")
                                return None
...
class CoachingStrategy:
    ...
    def __init__(self):
        ...
        self._add_worksmart_strategies()  # Add WorkSmart-specific patterns
    ...
    def select_strategy(self, context: Dict[str, float]) -> Optional[Dict]:
        """Select best coaching strategy based on context - AGGRESSIVE WorkSmart mode"""
        
        # CRITICAL PATTERNS (immediate intervention)
        if context.get('stress_level', 0) > 0.6 and context.get('energy_level', 1) < 0.4:
            strategy_key = 'high_stress_low_energy'
        elif context.get('session_duration_hours', 0) > 2.5:  # Every 2.5 hours, not 11!
            strategy_key = 'break_needed'
        
        # VALUE-CREATION PATTERNS (WorkSmart core)
        elif self._detect_activity_theater(context):
            strategy_key = 'activity_theater'
        elif self._detect_ai_underutilization(context):
            strategy_key = 'ai_underutilization'
        
        # FOCUS-INTEGRITY PATTERNS (lowered thresholds)
        elif context.get('browser_tab_count', 0) > 8:  # Was 15+, now 8+
            strategy_key = 'tab_overload'
        elif context.get('app_switches_per_hour', 0) > 40:  # Was 80+, now 40+
            strategy_key = 'context_switching'
        elif context.get('focus_quality', 1) < 0.5:  # Was 0.3, now 0.5
            strategy_key = 'low_focus_high_switches'
        
        # WELLBEING-PACE PATTERNS
        elif context.get('time_period') == 'afternoon' and context.get('energy_level', 1) < 0.6:  # Was 0.5
            strategy_key = 'afternoon_slump'
        elif context.get('cognitive_load', 0) > 0.6:  # Was 0.8, now 0.6
            strategy_key = 'cognitive_overload'
        
        # POSITIVE PATTERNS (proactive optimization)
        elif context.get('productivity_score', 0) > 0.7 and context.get('focus_quality', 0) > 0.7:
            strategy_key = 'high_productivity_flow'
        elif context.get('time_period') == 'morning' and context.get('energy_level', 0) > 0.6:
    ...
    def _detect_activity_theater(self, context: Dict) -> bool:
        """Detect activity theater pattern (high communication, low output)"""
        emails = context.get('emails_sent', 0)
        meetings = context.get('meetings_attended', 0)
        commits = context.get('commits_made', 0)
        tickets = context.get('tickets_closed', 0)
        
        # High communication activity but low deliverable output
        high_communication = emails > 20 or meetings > 3
        low_output = commits == 0 and tickets == 0
        
        return high_communication and low_output
    
    def _detect_ai_underutilization(self, context: Dict) -> bool:
        """Detect AI tool underutilization compared to peers"""
        user_ai_usage = context.get('ai_tool_usage', 0)
        peer_avg = context.get('peer_ai_usage_avg', 0)
        
        # User using AI significantly less than peers
        return user_ai_usage < 0.3 and peer_avg > 0.7
    
    def _add_worksmart_strategies(self):
        """Add WorkSmart-specific coaching strategies"""
        
        # VALUE-CREATION strategies
        self.strategies['activity_theater'] = [
            {
                'message': "High email/meeting activity detected but no deliverables today. Consider batching communication to focus on core work.",
                'priority': 1,
                'action': 'focus_on_deliverables',
                'duration': 120
            }
        ]
        
        self.strategies['ai_underutilization'] = [
            {
                'message': "Your peers using AI tools show 2x productivity. Try GitHub Copilot or ChatGPT for your current task.",
                'priority': 2,
                'action': 'ai_adoption',
                'duration': 10
            }
        ...
        # FOCUS-INTEGRITY strategies (lowered thresholds)
        self.strategies['tab_overload'] = [
            {
                'message': "8+ browser tabs detected. Close unnecessary tabs to improve focus and reduce cognitive load.",
                'priority': 2,
        ...
        self.strategies['context_switching'] = [
            {
                'message': "Frequent app switching detected (40+/hour). Pick ONE task for the next 25 minutes.",
                'priority': 2,
                'action': 'single_tasking',
                'duration': 25
            }
        ]
...
class AICoach:
    ...
    def __init__(self, model_path: Optional[str] = None):
        ...
        # Fast-path optimization for WorkSmart <100ms target
        self._pattern_cache = {}
        self._cache_ttl = 30  # 30 second cache for patterns
    ...
    def _analyze_context_intelligence(self, telemetry: Dict[str, Any]) -> Dict[str, Any]:
        """Advanced context analysis for intelligent interventions"""
        
        context = {
            'flow_state': self._detect_flow_state(telemetry),
            'crisis_mode': self._detect_crisis_mode(telemetry),
            'learning_mode': self._detect_learning_mode(telemetry),
            'meeting_mode': self._detect_meeting_mode(telemetry),
            'creative_work': self._detect_creative_work(telemetry),
            'productivity_level': self._assess_productivity_level(telemetry)
        }
        
        # Determine primary context
        context['primary_mode'] = self._determine_primary_context(context)
        context['intervention_tolerance'] = self._calculate_intervention_tolerance(context)
        
        return context
    
    def _detect_flow_state(self, telemetry: Dict[str, Any]) -> Dict[str, Any]:
        """Detect if user is in flow state - CRITICAL for user satisfaction"""
        
        indicators = {}
        
        # Low app switching (< 10/hour = focused)
        app_switches = telemetry.get('app_switches_per_hour', 50)
        indicators['low_switching'] = app_switches < 10
        
        # Consistent activity pattern
        keystroke_variance = telemetry.get('keystroke_variance', 0.5)
        indicators['consistent_activity'] = keystroke_variance < 0.3
        
        # High productivity score
        productivity = telemetry.get('productivity_score', 0.5)
        indicators['high_productivity'] = productivity > 0.75
        
        # Extended focus period
        focus_minutes = telemetry.get('deep_focus_minutes', 0)
        indicators['extended_focus'] = focus_minutes > 30
        
        # Quality focus
        focus_quality = telemetry.get('focus_quality', 0.5)
        indicators['quality_focus'] = focus_quality > 0.8
        
        # Few recent interruptions
        notifications_dismissed = telemetry.get('notifications_dismissed_recent', 0)
        indicators['minimal_interruptions'] = notifications_dismissed < 2
        
        # Calculate flow score
        flow_score = sum(indicators.values()) / len(indicators)
        
        flow_state = {
            'in_flow': flow_score > 0.6,
            'flow_intensity': flow_score,
            'protection_level': 'high' if flow_score > 0.8 else 'medium' if flow_score > 0.6 else 'none',
            'estimated_remaining_minutes': self._estimate_flow_duration(telemetry, flow_score)
        }
        
        return flow_state
    
    def _detect_crisis_mode(self, telemetry: Dict[str, Any]) -> Dict[str, Any]:
        """Detect emergency/crisis situations - be supportive, not nagging"""
        
        crisis_indicators = {}
        
        # Production alerts
        browser_title = telemetry.get('browser_tab_title', '').lower()
        crisis_indicators['production_alerts'] = any(word in browser_title for word in 
            ['error', 'alert', 'incident', 'outage', 'critical', 'emergency', 'pagerduty'])
        
        # Urgent communication
        crisis_indicators['urgent_slack'] = telemetry.get('slack_urgency', 'normal') == 'high'
        crisis_indicators['high_mentions'] = telemetry.get('mentions_count', 0) > 8
        
        # Stress indicators
        stress_level = telemetry.get('stress_level', 0.5)
        crisis_indicators['high_stress'] = stress_level > 0.8
        
        # Rapid context switching (crisis response pattern)
        app_switches = telemetry.get('app_switches_per_hour', 50)
        crisis_indicators['crisis_switching'] = app_switches > 80
        
        # After-hours pressure
        time_period = telemetry.get('time_period', 'day')
        voluntary = telemetry.get('voluntary_overtime', True)
        crisis_indicators['pressure_hours'] = time_period in ['night', 'weekend'] and not voluntary
        
        # Team dependency
        crisis_indicators['team_pressure'] = telemetry.get('team_depending_on_user', False)
        
        crisis_score = sum(crisis_indicators.values()) / len(crisis_indicators)
        
        return {
            'in_crisis': crisis_score > 0.4,
            'crisis_intensity': crisis_score,
            'support_needed': crisis_score > 0.6,
            'intervention_style': 'minimal_wellness' if crisis_score > 0.4 else 'normal'
        }
    
    def _detect_learning_mode(self, telemetry: Dict[str, Any]) -> bool:
        """Detect research/learning activities - allow more tabs"""
        
        browser_title = telemetry.get('browser_tab_title', '').lower()
        learning_keywords = ['documentation', 'tutorial', 'learn', 'guide', 'reference', 'docs', 
                           'stack overflow', 'medium', 'dev.to', 'github', 'api']
        
        return any(keyword in browser_title for keyword in learning_keywords)
    
    def _detect_meeting_mode(self, telemetry: Dict[str, Any]) -> bool:
        """Detect if user is in a meeting"""
        
        meeting_indicators = [
            telemetry.get('in_meeting', False),
            telemetry.get('microphone_active', False),
            telemetry.get('camera_active', False),
            'zoom' in telemetry.get('current_application', '').lower(),
            'teams' in telemetry.get('current_application', '').lower(),
            telemetry.get('keystrokes_per_min', 100) < 20  # Low typing during meetings
        ]
        
        return sum(meeting_indicators) >= 2
    
    def _detect_creative_work(self, telemetry: Dict[str, Any]) -> bool:
        """Detect creative work that needs inspiration - allow more tabs"""
        
        creative_apps = ['figma', 'sketch', 'photoshop', 'illustrator', 'canva', 'miro', 'whimsical']
        current_app = telemetry.get('current_application', '').lower()
        
        return any(app in current_app for app in creative_apps)
    
    def _assess_productivity_level(self, telemetry: Dict[str, Any]) -> str:
        """Assess current productivity level"""
        
        productivity_score = telemetry.get('productivity_score', 0.5)
    ...
    def _determine_primary_context(self, context: Dict[str, Any]) -> str:
        """Determine primary context for intervention decisions"""
        
        # Priority order - flow state trumps everything
        if context['flow_state']['in_flow']:
            return 'flow_state'
        elif context['crisis_mode']['in_crisis']:
            return 'crisis_mode'
        elif context['meeting_mode']:
            return 'meeting_mode'
        elif context['learning_mode']:
    ...
    def _calculate_intervention_tolerance(self, context: Dict[str, Any]) -> float:
        """Calculate how tolerant user is to interruptions (0-1)"""
        
        base_tolerance = 0.5
        
        # Flow state - very low tolerance
        if context['flow_state']['in_flow']:
            base_tolerance -= 0.4
        
        # Crisis mode - low tolerance for productivity coaching
        if context['crisis_mode']['in_crisis']:
            base_tolerance -= 0.3
        
        # Meeting mode - no tolerance
        if context['meeting_mode']:
            base_tolerance = 0.0
        
        # High productivity - lower tolerance
        if context['productivity_level'] == 'high':
            base_tolerance -= 0.2
        elif context['productivity_level'] == 'very_low':
            base_tolerance += 0.2
        
        return max(0.0, min(1.0, base_tolerance))
    
    def _estimate_flow_duration(self, telemetry: Dict[str, Any], flow_score: float) -> int:
        """Estimate remaining flow duration in minutes"""
        
        current_focus_minutes = telemetry.get('deep_focus_minutes', 0)
        energy_level = telemetry.get('energy_level', 0.5)
        
        # High flow + high energy = longer estimated duration
        base_duration = 60 if flow_score > 0.8 else 30
        energy_multiplier = energy_level * 1.5
        
        return int(base_duration * energy_multiplier)
    
    def _get_context_thresholds(self, context: Dict[str, Any]) -> Dict[str, int]:
        ...
        thresholds = {
            'flow_state': {
                'tab_count': 999,  # No limit during flow
                'session_hours': 6.0,  # Allow longer sessions
                'app_switches': 999    # No limit during flow
            },
            'crisis_mode': {
                'tab_count': 50,       # High limit during crisis
                'session_hours': 12.0, # Emergency sessions allowed
                'app_switches': 120    # High limit during crisis response
            },
            'learning_mode': {
                'tab_count': 20,       # Still allow research but not unlimited
                'session_hours': 4.0,  # Learning sessions can be long
                'app_switches': 60     # Research involves switching
            },
            'creative_work': {
                'tab_count': 20,       # Creative work needs inspiration
                'session_hours': 4.0,  # Creative sessions can be long
                'app_switches': 40     # Moderate switching for inspiration
            },
            'meeting_mode': {
                'tab_count': 999,      # Don't interrupt meetings
                'session_hours': 999,  # Don't interrupt meetings
                'app_switches': 999    # Don't interrupt meetings
            },
            'normal_work': {
                'tab_count': 8,        # Standard threshold
                'session_hours': 2.5,  # Standard threshold
                'app_switches': 40     # Standard threshold
            }
        }
        
        return thresholds.get(primary_mode, thresholds['normal_work'])
    
    def _fast_pattern_check(self, telemetry: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Fast pattern detection with context intelligence"""
        ...
        # STEP 1: Analyze context intelligence
        context = self._analyze_context_intelligence(telemetry)
        
        # STEP 2: Check intervention tolerance
        if context['intervention_tolerance'] < 0.2:
            # User is in flow/crisis/meeting - minimal intervention only
            self._pattern_cache[str(telemetry)] = None
            return None
        
        # STEP 3: Get context-appropriate thresholds
        thresholds = self._get_context_thresholds(context)
        
        # STEP 4: Multi-factor validation before any intervention
        result = self._validate_intervention_with_context(telemetry, context, thresholds)
        
        # Cache result
        self._pattern_cache[str(telemetry)] = result
        return result
    
    def _validate_intervention_with_context(self, telemetry: Dict[str, Any], 
                                          context: Dict[str, Any], 
                                          thresholds: Dict[str, int]) -> Optional[Dict[str, Any]]:
        """Multi-factor validation to prevent false positives"""
        
        # CRITICAL: Activity theater detection (high confidence)
        emails = telemetry.get('emails_sent', 0)
        commits = telemetry.get('commits_made', 0)
        if (emails > 20 and commits == 0):  # Remove productivity check - catch earlier
            
            return {
                'message': f"High email activity ({emails} emails) but no deliverables today. Focus on core work outputs.",
                'action': 'focus_on_deliverables',
                'priority': 1,
                'confidence': 0.9,
                'ai_powered': False,
                'intervention_type': 'value_creation'
            }
        
        # CRITICAL: AI underutilization (high confidence)
        ai_usage = telemetry.get('ai_tool_usage', 1)
        peer_avg = telemetry.get('peer_ai_usage_avg', 0)
        if (ai_usage < 0.5 and peer_avg > 0.7):  # Removed productivity check
            
            return {
                'message': f"Peers using AI tools show 2x productivity (you: {ai_usage*100:.0f}%, peers: {peer_avg*100:.0f}%). Consider GitHub Copilot.",
                'action': 'ai_adoption',
        ...
        tab_count = telemetry.get('browser_tab_count', 0)
        if tab_count > thresholds['tab_count']:
            # Multi-factor validation for tabs
            confidence_factors = []
            
            # High productivity = lower confidence in tab intervention
            if context['productivity_level'] == 'high':
                confidence_factors.append(-0.4)
            
            # Learning mode = research needs tabs
            if context['learning_mode']:
                confidence_factors.append(-0.3)
            
            # Creative work = inspiration needs tabs
            if context['creative_work']:
                confidence_factors.append(-0.2)
            
            base_confidence = 0.8
            final_confidence = base_confidence + sum(confidence_factors)
            
            if final_confidence > 0.5:  # Only intervene if confident
                return {
                    'message': f"{tab_count} tabs detected. Close unnecessary tabs to improve focus.",
                    'action': 'tab_management',
                    'priority': 2,
                    'confidence': final_confidence,
                    'ai_powered': False,
                    'intervention_type': 'focus_integrity'
                }
        
        # Context-aware app switching
        app_switches = telemetry.get('app_switches_per_hour', 0)
        if app_switches > thresholds['app_switches']:
            # Validate against context
            if (context['primary_mode'] not in ['crisis_mode', 'learning_mode'] and
                context['productivity_level'] in ['low', 'very_low']):
                
                return {
                    'message': "Frequent app switching detected. Pick ONE task for the next 25 minutes.",
                    'action': 'single_tasking',
                    'priority': 2,
                    'confidence': 0.8,
                    'ai_powered': False,
        ...
        if session_hours > thresholds['session_hours']:
            # Validate break need
            energy = telemetry.get('energy_level', 0.5)
            stress = telemetry.get('stress_level', 0.5)
            
            # Only suggest breaks if user shows fatigue signs
            if energy < 0.4 or stress > 0.7:
                priority = 1 if (energy < 0.3 or stress > 0.8) else 2
                
                return {
                    'message': f"You've been working for {session_hours:.1f} hours. Take a 10-minute break to maintain peak performance.",
                    'action': 'break_reminder',
                    'priority': priority,
                    'confidence': 0.9,
                    'ai_powered': False,
                    'intervention_type': 'wellbeing_pace'
                }
        
        # Energy/focus interventions (only if really needed)
        energy = telemetry.get('energy_level', 0.5)
        focus = telemetry.get('focus_quality', 0.5)
        time_period = telemetry.get('time_period', 'day')
        
        # Afternoon slump (less restrictive)
        if (time_period == 'afternoon' and energy < 0.4 and focus < 0.5):  # Raised thresholds
        ...
        # Positive feedback for good productivity
        if (context['productivity_level'] == 'high' and 
            telemetry.get('session_duration_hours', 0) > 6 and
            telemetry.get('commits_made', 0) > 0):
            
            return {
                'message': "Great productive day! You've made solid progress. Consider wrapping up soon.",
                'action': 'positive_feedback',
                'priority': 3,  # Low priority, positive message
                'confidence': 0.8,
                'ai_powered': False,
    ...
    async def analyze_telemetry(self, telemetry: Dict[str, Any], 
        ...
        """
        Main AI coaching interface with fast-path optimization
        ...
        """
        try:
            # FAST PATH: Check for immediate patterns first (<100ms target)
            fast_result = self._fast_pattern_check(telemetry)
            if fast_result:
                return fast_result
            ...
            if self.ai_client:
                try:
                    ...
                    logger.debug(f"🔍 AI client returned: {ai_recommendation}")
                    ...
                    if ai_recommendation and ai_recommendation.get('nudge_text'):
                        ...
                        if ai_recommendation.get('source') == 'ai_fusion':
                            ...
                            logger.info(f"🤖 Using AI analysis: {ai_recommendation.get('nudge_type', 'unknown')}")
                    else:
                        logger.warning(f"❌ AI recommendation missing nudge_text: {ai_recommendation}")
                        ai_recommendation = None
                except Exception as e:
