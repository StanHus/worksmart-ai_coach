...
class MicroInterventionSystem:
    ...
    def evaluate_micro_intervention(self, context: Dict, analysis: Dict, 
        ...
        app_switches = self._count_recent_app_switches(context_history, minutes=3)
        if app_switches > 4:
            return self._create_micro_intervention(
                InterventionType.MICRO_NUDGE,
        ...
        if session_minutes > 45 and productivity_score > 0.8:
            return self._create_micro_intervention(
                ...
                "Energy management",
                "Excellent sustained productivity! Consider taking a 2-minute break to maintain this level.",
    ...
    def _create_micro_intervention(self, intervention_type: InterventionType, 
        ...
        intervention = {
            ...
            "notification_priority": min(3, intensity.value),  # Cap at priority 3
    ...
    def _is_in_flow_state(self, context: Dict, analysis: Dict) -> bool:
        ...
        chrome_url = context.get('chrome_context', {}).get('active_tab_url', '')
    ...
    def _detect_pre_distraction_pattern(self, context: Dict, 
        ...
        if 'google.com/search' in chrome_url:
            recent_urls = [c.get('chrome_url', '') for c in context_history[-5:]]
            google_searches = sum(1 for url in recent_urls if 'google.com/search' in url)
            if google_searches >= 2:
                return True
        
        # Pattern 2: Social media browsing initiation
        social_indicators = ['linkedin.com', 'twitter.com', 'facebook.com', 'reddit.com']
        ...
        if current_tabs > 8 and context_history:
            previous_tabs = context_history[-1].get('tab_count', 0) if context_history else 0
    ...
    def adjust_sensitivity(self, feedback: str) -> None:
        ...
        if "too many" in feedback.lower() or "annoying" in feedback.lower():
            ...
            print(f"🔧 Increased intervention cooldown to {self.intervention_cooldown_minutes} minutes")
        
        elif "more" in feedback.lower() or "helpful" in feedback.lower():
            self.intervention_cooldown_minutes = max(2, self.intervention_cooldown_minutes - 0.5)
            print(f"🔧 Decreased intervention cooldown to {self.intervention_cooldown_minutes} minutes")
