...
def status(ctx):
    ...
    try:
        result = subprocess.run(
            ['pgrep', '-f', 'worksmart'], capture_output=True, text=True)
        worksmart_running = len(result.stdout.strip()) > 0
        click.echo(
            f"WorkSmart: {'🟢 Running' if worksmart_running else '🔴 Not running'}")
    except:
    ...
    for filename in log_files:
        if os.path.exists(filename):
            ...
            click.echo(
                f"Log file: {filename} (last updated: {mtime.strftime('%H:%M:%S')})")
...
def stats(days):
    ...
    stat_files = ['daily_stats.json'] if os.path.exists(
        'daily_stats.json') else []
    ...
    for file in sorted(stat_files)[-days:]:
        try:
            ...
            click.echo(
                f"{date}: {hours:.1f}h, Productivity: {productivity:.1%}, Focus: {focus:.1%}")
        except:
            continue
