        ...
        while self.running:
            try:
                ...
                if telemetry:
                    ...
                    print(f"   Session: {telemetry.get('session_duration_hours', 0):.1f} hours")
                    print(f"   Productivity: {telemetry.get('productivity_score', 0):.1%}")
                    
                    self._write_status("analyzing", "Processing telemetry with AI")
                    ...
                    if coaching_result:
                        print(f"🤖 AI Coaching generated: {coaching_result.get('message', '')[:50]}...")
                        self._write_coaching(coaching_result)
                        self._write_status("coaching_delivered", f"Delivered coaching to user")
                    else:
                        self._write_status("no_coaching", "No coaching needed at this time")
